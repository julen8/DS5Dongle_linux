...
static void handle_ctl_events(snd_ctl_t *ctl) {
    ...
    while ((err = snd_ctl_read(ctl, event)) > 0) {
        ...
        if (strcmp(name, CTL_ELEM_NAME) != 0) continue;

        /* 只关心 VALUE 变化; 也可能收到 INFO/ADD/REMOVE 等 */
        if (!(mask & SND_CTL_EVENT_MASK_VALUE)) continue;
    ...
    if (err < 0 && err != -EAGAIN) fprintf(stderr, "snd_ctl_read: %s\n", snd_strerror(err));
}
...
int main(void) {
    ...
    /* === 1. 打开并配置 PCM (同上一份示例) === */
    if ((err = snd_pcm_open(&pcm, PCM_DEVICE, SND_PCM_STREAM_CAPTURE, SND_PCM_NONBLOCK)) < 0) {
        fprintf(stderr, "pcm_open: %s\n", snd_strerror(err));
        return 1;
    }
    if ((err = snd_pcm_set_params(pcm, FORMAT, SND_PCM_ACCESS_RW_INTERLEAVED, CHANNELS, SAMPLE_RATE, 1, 100 * 1000)) < 0) {
    ...
    for (;;) {
        ...
        if (pcm_revents & POLLERR) {
            if (snd_pcm_state(pcm) == SND_PCM_STATE_XRUN) snd_pcm_prepare(pcm), snd_pcm_start(pcm);
        ...
        if (ctl_revents & (POLLIN | POLLERR)) handle_ctl_events(ctl);
    }
